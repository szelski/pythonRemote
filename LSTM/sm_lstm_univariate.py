...
class WriteResultInDb():
    ...
    def handle(self, context):
        ...
        data_to_insert = [
            (current_date, current_time, type, str(context.data.iloc[0, 4]), context.resultID, str(scalar_value)),
        ]

        context.cursor.executemany('INSERT INTO ' + context.symbol +
                                   'AI_RESULT_TMP (DATE, TIME, TYPE, RECT_ID, RESULT_ID, RESULT) '
                                   'VALUES (?, ?, ?, ?, ?, ?)',
                                   data_to_insert)
        context.cursor.executemany('INSERT INTO ' + context.symbol +
                                   'AI_RESULT (DATE, TIME, TYPE, RECT_ID, RESULT_ID, RESULT) '
                                   'VALUES (?, ?, ?, ?, ?, ?)',
                                   data_to_insert)
        ...
        logger.info("predicted buy: " + str(scalar_value))
...
class MyContext(Context):
    ...
    cursor = conn.cursor()
    symbol = "NAS100_0_"
    getDataQuery = "SELECT * FROM " + symbol + "AI_DATA_TMP"
    deleteDataQuery = "DELETE FROM " + symbol + "AI_DATA_TMP"
    sequence_length = "20"
    query_sequence = "SELECT POINTS_SPAN FROM " + symbol + "AI_DATA_BUY ORDER BY ROWID DESC LIMIT " + sequence_length
    data = 0
    data_tailored = 0
    resultID = 0
